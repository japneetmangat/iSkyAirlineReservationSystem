...
public class InitializeISkyController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        wireShowHidePassword(chkShowPwd2, pwd2, txtPwd2Visible); // show/hide pattern [web:68]
    ...
    private void handleCreateMaster() {
        ...
        if (userAccountService.isInitialized()) {
            setStatus("System is already initialized.", true);
            return;
        }

        try {
            String username = txtMasterUser.getText().trim();
            String password = getPwd1();

            userAccountService.createMaster(username, password);

            // Navigate to Login.fxml
            SceneNavigator.setRoot(btnCreateMaster.getScene(), "/se2203b/assignments/views/Login.fxml","iSky Authentication", 400.0, 500.0); // scene swap

        } catch (Exception ex) {
            setStatus("Unable to create master account. Please verify the inputs." +"\n"+ex, true);
            setPwd1("");
            setPwd2("");
        } finally {
            busy.set(false);
    ...
    public boolean isMasterExist() {
        return userAccountService.isInitialized();
    }
}
